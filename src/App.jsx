...
const initialWords = ['calor', 'actor', 'agudo', 'altar', 'lapiz', 'salto', 'ebrio', 'nadie', 'labor', 'abeja'];
...
export function Game() {
  ...
  const [inputValues, setInputValues] = useState(Array(originalWord.length).fill(''));
  ...
  useEffect(() => {
    // Shuffle the word when the component loads
    let scrambled = '';
    do {
    ...
    if (/^[A-Za-z]*$/.test(currentValue)) {
      if (currentValue === originalWord[currentInputIndex]) {
        if (currentInputIndex === originalWord.length - 1) {
          ...
          const newWords = originalWords.filter((word) => word !== originalWord);
  ...
  const handleDelete = () => {
    setInputValues((prevInputValues) => {
      ...
      updatedInputValues[currentInputIndex] = ''; // Delete the letter
  ...
  const inputBoxes = inputValues.map((value, index) => {
    ...
    return (
      <div key={index} className="flex flex-col-reverse items-center justify-center">
        <input
          type="text"
          value={value}
          onChange={isDisabled ? null : handleInputChange}
          disabled={isDisabled}
          className={`w-32 text-center border-2 border-slate-700 bg-slate-400 ${isDisabled ? 'cursor-not-allowed' : ''}`}
        />
        {index === currentInputIndex && !isDisabled && (
          <button onClick={handleDelete} className="absolute p-1 rounded text-white bg-red-600 bottom-[17.6rem]">
            Borrar
          </button>
        )}
      </div>
  ...
  return (
    <div className='flex flex-col items-center justify-center h-screen bg-slate-500'>
      <div className='bg-slate-200 p-6 rounded flex flex-col justify-center items-center'>
        ...
        <div className='flex flex-row items-center justify-center w-full mt-10 gap-x-36'>
          <p className='p-2 font-black text-black bg-blue-400 rounded'>Palabra desordenada: {scrambledWord}</p>
          <p className='p-2 font-black text-black bg-blue-400 rounded'>Vidas: {lives}</p>
        </div>
        ...
        <button onClick={handleRestart} className='p-2 mt-32 rounded text-white bg-black'>Reiniciar</button>
